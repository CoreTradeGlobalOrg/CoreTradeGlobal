/**
 * Lawyer Client Channels Page
 *
 * URL: /lawyer/channels
 * Access: lawyer, admin (enforced by middleware)
 *
 * Shows all lawyer engagements in a channel-centric list.
 * Each card links through to /deals/{dealId}/legal for active/completed engagements.
 */
...
import { LawyerChannels } from '@/presentation/components/features/legal/LawyerChannels/LawyerChannels';
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="flex items-center gap-3">
          ...
          <div className="space-y-2">
            <div className="h-6 w-40 bg-[rgba(255,255,255,0.06)] rounded animate-pulse" />
...
function LawyerChannelsContent() {
  ...
  useEffect(() => {
    if (!authLoading && !isAuthenticated) {
      router.push('/login?redirect=/lawyer/channels');
    }
  ...
  if (user.role !== ROLES.LAWYER && user.role !== ROLES.ADMIN) {
    return (
      <div className="min-h-screen bg-[#0F1C2E] pt-[100px] flex items-center justify-center">
        <p className="text-[#8899AA] text-sm">Access denied. Redirecting...</p>
      </div>
    );
  }

  return <LawyerChannels />;
}
