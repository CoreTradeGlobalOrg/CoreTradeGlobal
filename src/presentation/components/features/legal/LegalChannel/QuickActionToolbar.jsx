...
function RiskInlineForm({ onSubmit, onCancel }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({ title: title.trim(), description: description.trim(), severity, status: 'open' });
    setTitle('');
    setDescription('');
    setSeverity(RISK_SEVERITY.MEDIUM);
  };
    ...
    >
      ...
      <div className="space-y-2">
        <input
          ...
          required
          className="w-full px-3 py-2 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] rounded-lg text-sm text-white placeholder-[#4A5B6E] focus:outline-none focus:border-purple-500/60"
        />
        <textarea
          value={description}
          ...
          required
          rows={2}
          className="w-full px-3 py-2 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] rounded-lg text-sm text-white placeholder-[#4A5B6E] focus:outline-none focus:border-purple-500/60 resize-none"
        />
        <div className="flex gap-2">
          <select
            value={severity}
            onChange={(e) => setSeverity(e.target.value)}
            className="flex-1 px-3 py-2 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] rounded-lg text-sm text-white focus:outline-none focus:border-purple-500/60"
          >
          ...
          <button
            ...
            className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-[#0F1C2E] font-semibold rounded-lg text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Add Risk
          </button>
        </div>
