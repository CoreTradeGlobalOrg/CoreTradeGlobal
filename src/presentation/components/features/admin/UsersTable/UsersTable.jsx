...
import { Star, MoreVertical, Check, Ban, Trash2, Eye, Shield, ShieldX, ChevronDown, UserPlus, RefreshCw, Mail } from 'lucide-react';
...
export function UsersTable({ users = [], onRefresh }) {
  ...
  const [filterRole, setFilterRole] = useState('all'); // 'all', 'admin', 'member', 'logistics_provider', etc.
  ...
  // Handle toggle admin role — now calls setUserRole Cloud Function (not direct Firestore write)
  const handleToggleAdmin = async (user) => {
    const isCurrentlyAdmin = user.role === 'admin';
    const newRole = isCurrentlyAdmin ? ROLES.MEMBER : ROLES.ADMIN;

    setActionLoading(user.id);
    try {
      const setUserRoleFn = httpsCallable(functions, 'setUserRole');
      await setUserRoleFn({ userId: user.id, role: newRole });
  ...
  // Role badge for invite table
  const getRoleBadgeClasses = (role) => {
    const colors = ROLE_BADGE_COLORS[role] || ROLE_BADGE_COLORS[ROLES.MEMBER];
    // Map Tailwind light classes to dark-theme friendly versions
    const darkMap = {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    // Role filter — now supports all 5 roles
    const matchesRole =
      filterRole === 'all' ||
    ...
    >
      ...
      <div className="px-4 md:px-6 py-4 border-b border-[rgba(255,255,255,0.1)]">
        <div className="flex flex-col gap-4">
          ...
          {/* Filters — shown only on Users tab */}
          {activeTab === 'users' && (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0 md:flex-wrap">
                ...
                <select
                  ...
                  className="px-3 py-2 bg-[#1a2a3a] border border-[rgba(255,255,255,0.2)] rounded-lg text-white focus:border-[#FFD700] focus:outline-none cursor-pointer text-sm flex-shrink-0 min-w-[130px]"
                >
                  <option value="all" className="bg-[#1a2a3a] text-white">All Roles</option>
                  <option value={ROLES.MEMBER} className="bg-[#1a2a3a] text-white">Members</option>
                  <option value={ROLES.LOGISTICS_PROVIDER} className="bg-[#1a2a3a] text-white">Logistics Providers</option>
                  <option value={ROLES.INSURANCE_PROVIDER} className="bg-[#1a2a3a] text-white">Insurance Providers</option>
                  <option value={ROLES.LAWYER} className="bg-[#1a2a3a] text-white">Lawyers</option>
                  <option value={ROLES.ADMIN} className="bg-[#1a2a3a] text-white">Admins</option>
                </select>
              </div>
            </div>
          )}
                  ...
                  <tr key={invite.id} className="hover:bg-[rgba(255,255,255,0.03)] transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-400">
                        {invite.invitedAt ? invite.invitedAt.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : 'N/A'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
